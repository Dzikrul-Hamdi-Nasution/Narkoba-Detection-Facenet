...
kontur_wajah = ['Heroin', 'Ganja', 'Sabu', 'Up Scale Sabu', 'Up Scale Ganja', 'Up Scale Heroin', 'Normal']

cv2.namedWindow('Prediction_Face')
camera = cv2.VideoCapture(0)  ## to use laptop camera 
...
preds=[]
out = cv2.VideoWriter()
out.open('output_Prediction.mp4',fourcc, 15, sz, True) # initialize the writer


while True: # when reading from a video camera
    color_frame = camera.read()[1]
    color_frame = imutils.resize(color_frame,width=min(480, color_frame.shape[1]))
    ...
    if len(detected_faces)>0:
        ...
        label = kontur_wajah[preds.argmax()]
    ...
    for (i, (emotion, prob)) in enumerate(zip(kontur_wajah, preds)):
